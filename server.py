#!/usr/bin/env python3
"""
Test Full Deploy 1772170610 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('test-full-deploy-1772170610_mcp')
...
logger.info("="*80)
logger.info(f"Test Full Deploy 1772170610 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Test Full Deploy 1772170610 MCP Server", host="0.0.0.0")
...
# - TEST_FULL_DEPLOY_1772170610_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-full-deploy-1772170610-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "test-full-deploy-1772170610-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Test Full Deploy 1772170610 MCP Server")
    logger.info("="*80)
